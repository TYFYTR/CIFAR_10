...
DROPOUT_RATE = 0.5
USE_TTA = False                   # Test-Time Augmentation toggle
...
# Store original test images for TTA (before preprocessing removes them)
original_test_images = None
if USE_TTA:
    original_test_images = [item['image'] for item in dataset['test']]
...
def predict_with_tta(model, processor, images, device, batch_size=BATCH_SIZE):
    """
    Apply Test-Time Augmentation: create 5 augmented versions of each image,
    run model on all versions, and average the logits.
    
    Augmentations:
    1. Original image
    2. Horizontal flip
    3. Rotate 10 degrees (deterministic)
    4. Slightly different crop (deterministic)
    5. Color jitter (deterministic)
    """
    from PIL import Image
    ...
    # Create deterministic TTA transforms
    # For deterministic augmentations, we'll apply them with fixed parameters
    def crop_transform(img):
        """Apply a deterministic crop - take center crop with slight offset"""
        w, h = img.size
        # Crop from slightly off-center
        crop_size = min(w, h) * 0.9
        left = (w - crop_size) / 2 + w * 0.05
        top = (h - crop_size) / 2 + h * 0.05
        return functional.crop(img, int(top), int(left), int(crop_size), int(crop_size))
    
    def color_jitter_transform(img):
        """Apply deterministic color jitter with fixed factors"""
        # Use fixed factors based on config (slightly brighter, more contrast, etc.)
        img = functional.adjust_brightness(img, 1.0 + BRIGHTNESS)
        img = functional.adjust_contrast(img, 1.0 + CONTRAST)
        img = functional.adjust_saturation(img, 1.0 + SATURATION)
        img = functional.adjust_hue(img, HUE)
        return img
    
    tta_transforms = [
        lambda img: img,  # Original (no transform)
        lambda img: functional.hflip(img),  # Horizontal flip
        lambda img: functional.rotate(img, ROTATION_DEGREES),  # Rotate 10 degrees
        lambda img: functional.resize(crop_transform(img), (224, 224)),  # Different crop
        color_jitter_transform,  # Color jitter
    ]
    
    # Process each image with all 5 augmentations
    with torch.no_grad():
        for img in images:
            img_logits = []
            ...
            # Apply each augmentation and get logits
            for tta_transform in tta_transforms:
                # Apply augmentation
                augmented_img = tta_transform(img_pil)
                
                # Process through model
                inputs = processor(augmented_img, return_tensors='pt')
                inputs = {k: v.to(device) for k, v in inputs.items()}
                
                # Get logits
                outputs = model(**inputs)
                logits = outputs.logits
                img_logits.append(logits.cpu())
            
            # Average logits across all augmentations
            avg_logits = torch.stack(img_logits).mean(dim=0)
...
# Compute test accuracy with TTA if enabled (for display)
test_accuracy_tta = None
if USE_TTA and original_test_images is not None:
    print("Computing test accuracy with TTA...")
    model.to(device)
    tta_logits = predict_with_tta(model, processor, original_test_images, device)
    test_labels = [item['labels'].item() if torch.is_tensor(item['labels']) else item['labels'] 
                   for item in dataset['test']]
    y_true_tta = np.array(test_labels)
    y_pred_tta = tta_logits.numpy().argmax(-1)
    test_accuracy_tta = accuracy_score(y_true_tta, y_pred_tta)
    print(f"Training:   {train_results['eval_accuracy']:.1%}")
    print(f"Validation: {val_results['eval_accuracy']:.1%}")
    print(f"Test:       {test_results['eval_accuracy']:.1%} (baseline)")
    print(f"Test (TTA): {test_accuracy_tta:.1%} (with TTA)")
else:
    print(f"Training:   {train_results['eval_accuracy']:.1%}")
    print(f"Validation: {val_results['eval_accuracy']:.1%}")
...
# Get predictions with confidence (use TTA if enabled)
if USE_TTA and original_test_images is not None:
    print("Applying Test-Time Augmentation to test set...")
    model.to(device)
    tta_logits = predict_with_tta(model, processor, original_test_images, device)
    tta_predictions_np = tta_logits.numpy()
    
    # Get labels from test dataset
    test_labels = [item['labels'].item() if torch.is_tensor(item['labels']) else item['labels'] 
                   for item in dataset['test']]
    y_true = np.array(test_labels)
    y_pred = tta_predictions_np.argmax(-1)
    logits = tta_logits
else:
...
# Calculate confusion matrix (y_pred and y_true are already set above)
...
results = {
    ...
    "metrics": {
        ...
        "test_tta": round(test_accuracy_tta, 4) if (USE_TTA and original_test_images is not None) else None,
